...
class Segmentation(BaseComponent):
    ...
    def process(self, image):
        """
        Perform segmentation on the input image without applying watershed.
        ...
        # Step 6: Create markers from connected components
        _, markers = cv2.connectedComponents(cv2.bitwise_not(edges))
        self.save_temp_result((markers * 10).astype('uint8'), "final_markers")  # Visualize markers

        # Return markers as the segmentation result
        return markers
